...
(() => {
  ...
  function renderRbcSettings(){
    ...
    $('#view-module').innerHTML=`<div class="page-head"><div><h1>ตั้งค่า RBC QC</h1><p class="muted">LPRC / LDPRC</p></div></div><div class="panel"><h2>เกณฑ์ QC</h2><div class="form-grid">${rbcField('LPRC Residual WBC สูงสุด (×10⁹)','rs_lprc_wbc',s.lprc_residual_wbc_max_x10e9,'number',false,false,'0.1')}${rbcField('LPRC RBC Recovery ขั้นต่ำ (%)','rs_lprc_rec',s.lprc_rbc_recovery_min_pct,'number',false,false,'0.1')}${rbcField('LDPRC Residual WBC สูงสุด (×10⁶)','rs_ldprc_wbc',s.ldprc_residual_wbc_max_x10e6,'number',false,false,'0.1')}${rbcField('LDPRC RBC Recovery ขั้นต่ำ (%)','rs_ldprc_rec',s.ldprc_rbc_recovery_min_pct,'number',false,false,'0.1')}${rbcField('Hct ขั้นต่ำ (%)','rs_hct_min',s.hct_min_pct,'number',false,false,'0.1')}${rbcField('Hct สูงสุด (%)','rs_hct_max',s.hct_max_pct,'number',false,false,'0.1')}<div class="field span2"><label>&nbsp;</label><label class="inline-check"><input id="rs_two" type="checkbox" ${s.require_two_measurements?'checked':''}> ต้องมีผลครบ 2 ครั้ง</label><label class="inline-check"><input id="rs_pre_ev" type="checkbox" ${s.require_pre_evidence?'checked':''}> บังคับหลักฐาน CBC ก่อน</label><label class="inline-check"><input id="rs_post_ev" type="checkbox" ${s.require_post_evidence?'checked':''}> บังคับหลักฐาน CBC หลัง</label><label class="inline-check"><input id="rs_post_adam_ev" type="checkbox" ${s.require_post_adam_evidence!==false?'checked':''}> บังคับหลักฐาน ADAM หลัง (LDPRC)</label></div></div><div class="actions"><button class="btn primary" id="saveRbcSettings">บันทึกเกณฑ์</button></div></div>
