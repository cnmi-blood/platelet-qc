/* CNMI Blood Component QC · Platelet module v4.7.0 - plain JS / Supabase */
(() => {
  'use strict';
  ...
  const state = { sb:null, session:null, user:null, profile:null, settings:null, productSettings:[], records:[], profiles:[], currentRecordId:null, currentEvidence:[], currentPool:[], lastLoginPassword:null, uiMode:'staff', auditUserFilter:'', resetTargetId:null };
  ...
  async function logActivity(action,entityType='system',recordId=null,detail={}){
    ...
    const payload={app_version:'4.7.0',ui_mode:state.uiMode,...detail};
  ...
  function exportCSV(rows){
    const headers=['Product No.','Product','Group','Collection','Expiry','Gross weight g','Bag tare g','Density','Volume mL','Pool PYI','PLT instrument','PLT1','PLT2','PLT measured at','PLT used','Yield x10^11','Equivalent Units','WBC ADAM','WBC measured at','Residual WBC x10^6','pH','pH measured','QC','Status','Revision','Notes'];
    const vals=r=>[r.product_no,r.product_type,r.blood_group,r.collection_at,r.expiry_at,r.gross_weight_g,r.bag_tare_weight_g,r.density,r.volume_ml,r.pool_pyi,r.plt_instrument,r.plt_value_1,r.plt_value_2,r.plt_measured_at,r.plt_used,r.platelet_yield,r.equivalent_units,r.wbc_adam,r.wbc_measured_at,r.residual_wbc,r.ph_value,r.ph_measured_at,r.qc_status,r.status,r.revision,r.notes];
  ...
  async function renderRecordForm(){
    ...
    setEditable(editable); applyProductWeightConfig(r); togglePool(); updateCalcPreview(); renderEvidenceLists(r?.id,editable);
  ...
  async function saveRecord(silent=false,auto=false){
    try{ const payload=collectRecord(); if(!payload.product_no||!payload.product_type){if(auto)showToast('กรอก Product No. และผลิตภัณฑ์ก่อนแนบไฟล์','error');else showToast('กรุณากรอก Product No. และผลิตภัณฑ์','error');return false;} let id=state.currentRecordId;
      ...
      if(productSetting(payload.product_type)?.requires_pool || payload.product_type?.startsWith('LDPPC ')) await savePool(id); else {await state.sb.from('pool_units').delete().eq('record_id',id);}
  ...
  async function renderSettings(){
    ...
    $('#view-settings').innerHTML=`<div class="page-head"><div><h1>จัดการระบบ Platelet QC</h1><p class="muted">จัดการผู้ใช้งาน สิทธิ์การเข้าถึง รหัสผ่าน และเกณฑ์ QC ของการเตรียมเกล็ดเลือด</p></div><div class="actions"><button class="btn" id="openAuditFromSettings">ประวัติการใช้งาน</button></div></div>
      ...
      <div class="notice warning"><strong>ก่อนเริ่มใช้ระบบ Platelet QC จริง:</strong> ตรวจสอบทั้งค่าน้ำหนักถุง/Density และเกณฑ์ QC ของเกล็ดเลือดให้ตรงกับ WI และข้อกำหนดที่หน่วยอนุมัติ การเปลี่ยนค่าจะมีผลกับรายการใหม่และถูกบันทึกใน Audit Log</div>
