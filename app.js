/* CNMI Blood Component QC v5.3.18 - simplified shared QC Fail / Corrective Action workflow */
(() => {
  'use strict';
  ...
  function updateRouteChrome(route){
    ...
    if(route.module){
      ...
      if(footer) footer.textContent=`CNMI Blood Component QC · ${meta.label} · v5.3.18 · bloodqc.cnmiblood.com${route.hash}`;
    }else{
      if(sub) sub.textContent='Blood Component Preparation & QC · CNMI Blood Bank';
      if(footer) footer.textContent='CNMI Blood Component QC · v5.3.18 · bloodqc.cnmiblood.com';
    }
  ...
  function renderNonconformanceDetail(n){
    ...
    $('#view-nonconformance').innerHTML=`<div class="page-head"><div><div class="breadcrumb"><button class="link-btn" id="ncBackList">QC ไม่ผ่าน / Corrective Action</button><span>›</span><span>${esc(n.nc_no)}</span></div><h1>${esc(n.nc_no)}</h1><p class="muted">${esc(ncModuleTH(n.module))} · ${esc(n.product_no)} · ${esc(n.product_type)}</p></div><div class="actions">${ncStatusBadge(n.status)}<button class="btn" id="ncOpenSource">ดูรายการ QC ต้นฉบับ</button></div></div>
      <div class="notice warning"><strong>หลักการสำคัญ:</strong> ระบบจะเก็บผล QC เดิมไว้เสมอ การตรวจซ้ำหรือการแก้ไขในหน้านี้จะไม่ทับข้อมูลเดิม</div>
      <div class="panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">1</span><div><h2>ข้อมูล QC ที่ไม่ผ่าน</h2><p class="muted small">ดูค่าที่ผิดเกณฑ์ก่อนเริ่มสอบสวน</p></div></div>${sourceQcBadge(n.module,r||{})}</div>${ncParamHtml(params)}<div class="nc-source-meta"><span>เลขผลิตภัณฑ์ <b>${esc(n.product_no)}</b></span><span>ชนิดผลิตภัณฑ์ <b>${esc(n.product_type)}</b></span><span>ผู้เปิดเคส <b>${esc(profileName(n.created_by))}</b></span><span>วันที่เปิดเคส <b>${esc(dateTH(n.created_at))}</b></span></div></div>
      <div class="panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">2</span><div><h2>ตรวจสอบผล <small>Verification</small></h2><p class="muted small">เช็กผล, ตัวอย่าง, เครื่องมือ และหลักฐาน</p></div></div></div><div class="form-grid"><div class="field"><label>สรุปผลการตรวจสอบ</label><select id="ncVerification" ${editable?'':'disabled'}>${enumOptions([['pending',NC_VERIFY_TH.pending],['technical_error',NC_VERIFY_TH.technical_error],['confirmed_failure',NC_VERIFY_TH.confirmed_failure]],n.verification_result,'เลือกผลการตรวจสอบ')}</select></div><div class="field span2"><label>บันทึกรายละเอียดการตรวจสอบ</label><textarea id="ncVerificationNote" ${editable?'':'disabled'} placeholder="เช่น ตรวจตัวอย่าง ตรวจเครื่อง ทบทวนการคำนวณ และดูหลักฐานครบแล้ว">${esc(n.verification_note||'')}</textarea></div><div class="field span2"><label>ผลตรวจซ้ำ (ถ้ามี) <small>Repeat</small></label><textarea id="ncRepeatResult" ${editable?'':'disabled'} placeholder="ระบุว่าตรวจซ้ำเมื่อไร ได้ผลอย่างไร และสรุปว่าอย่างไร">${esc(n.repeat_result||'')}</textarea></div></div></div>
      <div class="panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">3</span><div><h2>ประเมินผลกระทบ</h2><p class="muted small">ดูว่ากระทบถุงนี้ ถุงอื่น หรือผู้ป่วยหรือไม่</p></div></div></div><div class="form-grid"><div class="field"><label>สถานะถุงเมื่อพบผล</label><select id="ncProductStatus" ${editable?'':'disabled'}>${enumOptions([['in_stock','ยังอยู่ในคลัง'],['issued_not_transfused','จ่ายออกแล้ว แต่ยังไม่ให้ผู้ป่วย'],['transfused','ให้ผู้ป่วยแล้ว'],['destroyed','ทำลายแล้ว'],['unknown','ยังไม่ทราบ']],n.product_status,'เลือกสถานะถุง')}</select></div><div class="field"><label>จัดการถุงอย่างไร</label><select id="ncDisposition" ${editable?'':'disabled'}>${enumOptions([['quarantine','กักกัน / งดจ่าย'],['destroy','ทำลาย'],['release_with_approval','ปล่อยใช้ตามการอนุมัติ'],['notify_clinical','แจ้งหน่วยรักษา / ผู้เกี่ยวข้อง'],['no_action','ไม่ต้องดำเนินการกับผลิตภัณฑ์'],['other','อื่น ๆ']],n.disposition,'เลือกการจัดการ')}</select></div><div class="field span2"><label>รายละเอียดการจัดการ</label><textarea id="ncDispositionNote" ${editable?'':'disabled'} placeholder="อธิบายว่าทำอะไรกับถุงนี้บ้าง">${esc(n.disposition_note||'')}</textarea></div><div class="field span2"><label>ขอบเขตผลกระทบ <small>Scope assessment</small></label><textarea id="ncScope" ${editable?'':'disabled'} placeholder="เช่น กระทบเฉพาะถุงนี้ หรืออาจกระทบ lot / วันผลิต / เครื่อง / filter / ผู้ปฏิบัติงานเดียวกัน">${esc(n.scope_assessment||'')}</textarea></div><div class="field"><label>การแจ้งผู้เกี่ยวข้อง</label><select id="ncNotification" ${editable?'':'disabled'}>${enumOptions([['not_required','ไม่จำเป็น'],['required_pending','ต้องแจ้ง / รอดำเนินการ'],['completed','แจ้งแล้ว']],n.notification_status,'เลือกสถานะการแจ้ง')}</select></div><div class="field"><label>บันทึกการแจ้ง</label><input id="ncNotificationNote" ${editable?'':'disabled'} value="${esc(n.notification_note||'')}" placeholder="หน่วยหรือบุคคลที่แจ้ง / วันที่แจ้ง"></div></div></div>
      <div class="panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">4</span><div><h2>หาสาเหตุและแก้ไขเบื้องต้น</h2><p class="muted small">สรุปสาเหตุจริง และสิ่งที่ทำทันทีเพื่อควบคุมเหตุการณ์</p></div></div></div><div class="form-grid"><div class="field"><label>หมวดสาเหตุหลัก</label><select id="ncRootCause" ${editable?'':'disabled'}>${selectOptions(roots,n.root_cause_category,'เลือกหมวดสาเหตุ')}</select></div><div class="field span2"><label>รายละเอียดสาเหตุ <small>Root cause</small></label><textarea id="ncRootDetail" ${editable?'':'disabled'} placeholder="สรุปสาเหตุที่แท้จริงจากการสอบสวน">${esc(n.root_cause_detail||'')}</textarea></div><div class="field span2"><label>การแก้ไขเบื้องต้น <small>Correction</small></label><textarea id="ncCorrection" ${editable?'':'disabled'} placeholder="เช่น กักกันถุง ตรวจซ้ำ ตรวจเครื่อง เปลี่ยน consumable">${esc(n.immediate_correction||'')}</textarea></div></div></div>
      <div class="panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">5</span><div><h2>แผนแก้ไขป้องกันซ้ำ <small>Corrective Action</small></h2><p class="muted small">ระบุว่าจะทำอะไร เพื่อลดโอกาสเกิดซ้ำ</p></div></div></div><div class="form-grid"><div class="field span2"><label>แผนที่จะทำ</label><textarea id="ncCorrectiveAction" ${editable?'':'disabled'} placeholder="เช่น ปรับ WI, retrain, PM/สอบเทียบ, เปลี่ยน lot, เพิ่ม checkpoint">${esc(n.corrective_action||'')}</textarea></div><div class="field"><label>ผู้รับผิดชอบ</label><select id="ncResponsible" ${editable?'':'disabled'}>${profileOptions(n.responsible_by)}</select></div><div class="field"><label>กำหนดเสร็จ</label><input id="ncDueDate" type="date" ${editable?'':'disabled'} value="${esc(n.due_date||'')}"></div></div></div>
      <div class="panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">6</span><div><h2>ติดตามผล <small>Effectiveness</small></h2><p class="muted small">กำหนดว่าจะเช็กอย่างไรว่าแผนที่ทำได้ผล</p></div></div></div><div class="form-grid"><div class="field span2"><label>วิธีติดตามผล</label><textarea id="ncEffectMethod" ${editable?'':'disabled'} placeholder="เช่น ติดตาม QC ถัดไป 3 ครั้ง / ติดตาม 1 เดือน / audit การปฏิบัติตาม WI">${esc(n.effectiveness_method||'')}</textarea></div><div class="field"><label>กำหนดติดตาม</label><input id="ncEffectDue" type="date" ${editable?'':'disabled'} value="${esc(n.effectiveness_due_date||'')}"></div><div class="field"><label>ผลการติดตาม</label><select id="ncEffectResult" ${editable?'':'disabled'}>${enumOptions([['pending',NC_EFFECT_TH.pending],['effective',NC_EFFECT_TH.effective],['not_effective',NC_EFFECT_TH.not_effective]],n.effectiveness_result||'pending','เลือกผลการติดตาม')}</select></div><div class="field span2"><label>รายละเอียดผลติดตาม</label><textarea id="ncEffectNote" ${editable?'':'disabled'} placeholder="บันทึกผลที่พบหลังติดตาม">${esc(n.effectiveness_note||'')}</textarea></div>${n.effectiveness_checked_at?`<div class="field span2"><div class="readonly-box">ผู้ตรวจติดตามผล: ${esc(profileName(n.effectiveness_checked_by))} · ${esc(dateTH(n.effectiveness_checked_at))}</div></div>`:''}</div></div>
      <div class="panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">7</span><div><h2>หลักฐาน</h2><p class="muted small">แนบรูปหรือ PDF ที่เกี่ยวกับการตรวจสอบ การแก้ไข และการติดตามผล</p></div></div><span class="badge">${evidence.length} ไฟล์</span></div>${editable?`<div class="nc-evidence-uploader"><select id="ncEvidenceCategory">${enumOptions([['verification',NC_EVIDENCE_CATEGORY_TH.verification],['repeat',NC_EVIDENCE_CATEGORY_TH.repeat],['investigation',NC_EVIDENCE_CATEGORY_TH.investigation],['corrective_action',NC_EVIDENCE_CATEGORY_TH.corrective_action],['effectiveness',NC_EVIDENCE_CATEGORY_TH.effectiveness],['other',NC_EVIDENCE_CATEGORY_TH.other]],'verification','เลือกประเภทหลักฐาน')}</select><input class="hidden-file-input" type="file" id="ncEvidenceCamera" accept="image/*" capture="environment"><input class="hidden-file-input" type="file" id="ncEvidenceFile" accept="image/*,application/pdf"><button class="btn primary" type="button" id="ncEvidenceCameraBtn">ถ่ายรูป</button><button class="btn" type="button" id="ncEvidenceFileBtn">เลือกไฟล์ / PDF</button></div>`:''}<div id="ncEvidenceList" class="nc-evidence-list">${evidence.length?evidence.map(e=>`<div class="evidence-item"><span class="name evidence-name"><strong>${esc(e.original_name)}</strong><small>${esc(ncEvidenceCategoryLabel(e.category))} · ${esc(profileName(e.uploaded_by))} · ${esc(dateTH(e.created_at))}</small></span><span class="e-actions"><button class="btn small-btn nc-ev-view" data-id="${e.id}">ดู</button>${editable?`<button class="btn small-btn danger nc-ev-delete" data-id="${e.id}">ลบ</button>`:''}</span></div>`).join(''):'<div class="muted small">ยังไม่มีหลักฐาน</div>'}</div></div>
      <div class="panel nc-close-panel"><div class="section-title-row"><div class="nc-section-head"><span class="nc-step-no">8</span><div><h2>สรุปปิดเคส</h2><p class="muted small">ปิดเคสเมื่อข้อมูลครบและทบทวนเรียบร้อย</p></div></div></div><div class="field"><label>สรุปก่อนปิดเคส</label><textarea id="ncCloseNote" ${closed||canClose?'':'disabled'} ${closed?'disabled':''} placeholder="สรุปสิ่งที่พบ สิ่งที่ทำ และเหตุผลที่ปิดเคสได้">${esc(n.close_note||'')}</textarea></div>${closed?`<div class="notice good"><strong>ปิดเคสแล้ว</strong><br>${esc(profileName(n.closed_by))} · ${esc(dateTH(n.closed_at))}${n.close_note?`<br>${esc(n.close_note)}`:''}</div>`:''}</div>
      <div class="sticky-actions nc-sticky-actions"><div class="left"><button class="btn" id="ncBackBottom">กลับรายการ NC</button></div><div class="right">${editable?'<button class="btn primary" id="ncSave">บันทึก</button>':''}${canClose?'<button class="btn good" id="ncClose">ทบทวนและปิดเคส</button>':''}</div></div>`;
