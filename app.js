/* Platelet Preparation & QC v3.0.0 - plain JS / Supabase */
(() => {
  'use strict';
  ...
  function exportCSV(rows){
    const headers=['Product No.','Product','Group','Collection','Expiry','Volume mL','Pool PYI','PLT instrument','PLT1','PLT2','PLT used','Yield x10^11','Equivalent Units','WBC ADAM','Residual WBC x10^6','pH','pH measured','QC','Status','Revision','Notes'];
    const vals=r=>[r.product_no,r.product_type,r.blood_group,r.collection_at,r.expiry_at,r.volume_ml,r.pool_pyi,r.plt_instrument,r.plt_value_1,r.plt_value_2,r.plt_used,r.platelet_yield,r.equivalent_units,r.wbc_adam,r.residual_wbc,r.ph_value,r.ph_measured_at,r.qc_status,r.status,r.revision,r.notes];
  ...
  async function renderRecordForm(){
    ...
    $('#view-record').innerHTML=`
      <div class="page-head"><div><h1>${r?'แก้ไข / ตรวจรายการ':'บันทึกใหม่'}</h1><p class="muted">กรอกค่าที่วัดได้จากเครื่อง ระบบคำนวณผลให้เอง</p></div><div class="status-line">${r?statusBadge(r.status)+qcBadge(r.qc_status)+pHBadge(r):''}</div></div>
      ...
      <div class="panel"><h2>3. ผล Platelet จาก CBC</h2><div class="form-grid">
    ...
    $('#product_type').addEventListener('change',togglePool); ['collection_at','volume_ml','plt_value_1','plt_value_2','plt_use_mode','wbc_adam','ph_value','ph_measured_at'].forEach(id=>$('#'+id)?.addEventListener('input',updateCalcPreview));
  ...
  function evidenceBox(cat,title){ return `<div class="evidence-box"><h3>${title}</h3><div class="muted small">แนบรูปหรือ PDF จากเครื่อง</div><input type="file" id="file_${cat}" accept="image/*,application/pdf" style="margin-top:10px"><button type="button" class="btn small-btn" id="upload_${cat}" style="margin-top:8px">อัปโหลดหลักฐาน</button><div class="evidence-list" id="list_${cat}"></div></div>`; }
  function renderEvidenceLists(recordId,editable){ ['cbc','adam','ph'].forEach(cat=>{ const host=$('#list_'+cat); const arr=state.currentEvidence.filter(x=>x.category===cat); host.innerHTML=arr.length?arr.map(e=>`<div class="evidence-item"><span class="name" title="${esc(e.original_name)}">${esc(e.original_name)}</span><span class="e-actions"><button type="button" class="btn small-btn ev-view" data-id="${e.id}">ดู</button>${editable?`<button type="button" class="btn small-btn danger ev-del" data-id="${e.id}">ลบ</button>`:''}</span></div>`).join(''):'<div class="muted small">ยังไม่มีไฟล์</div>'; });
    $$('.ev-view').forEach(b=>b.onclick=()=>viewEvidence(b.dataset.id)); $$('.ev-del').forEach(b=>b.onclick=()=>deleteEvidence(b.dataset.id)); ['cbc','adam','ph'].forEach(cat=>{const b=$('#upload_'+cat);if(b){b.disabled=!editable;b.onclick=()=>uploadEvidence(cat);}});
  }
  ...
  function collectRecord(){ return {product_no:$('#product_no').value.trim(),product_type:$('#product_type').value,blood_group:$('#blood_group').value||null,collection_at:bangkokISO($('#collection_at').value),volume_ml:num($('#volume_ml').value),plt_instrument:$('#plt_instrument').value||null,plt_value_1:num($('#plt_value_1').value),plt_value_2:num($('#plt_value_2').value),plt_use_mode:$('#plt_use_mode').value,wbc_adam:num($('#wbc_adam').value),ph_value:num($('#ph_value').value),ph_measured_at:bangkokISO($('#ph_measured_at').value),ph_deviation_reason:$('#ph_deviation_reason').value.trim()||null,notes:$('#notes').value.trim()||null}; }
  ...
  async function openDetail(id){
    try{ const {data:r,error}=await state.sb.from('platelet_records').select('*').eq('id',id).single();if(error)throw error; const [{data:pool},{data:ev},{data:audit}]=await Promise.all([state.sb.from('pool_units').select('*').eq('record_id',id).order('position'),state.sb.from('evidence_files').select('*').eq('record_id',id).order('created_at'),state.sb.from('audit_logs').select('*').eq('record_id',id).order('created_at',{ascending:false}).limit(100)]);
      ...
      $('#detailBody').innerHTML=`<div class="status-line">${statusBadge(r.status)} ${qcBadge(r.qc_status)} ${pHBadge(r)}</div><div class="divider"></div><div class="detail-grid">${dcell('Group',r.blood_group)}${dcell('วัน-เวลาเริ่มเจาะ',dateTH(r.collection_at))}${dcell('วัน-เวลาหมดอายุ',dateTH(r.expiry_at))}${dcell('Volume',fmt(r.volume_ml,1)+' mL')}${dcell('Pool PYI',fmt(r.pool_pyi,2))}${dcell('เครื่อง CBC',r.plt_instrument)}${dcell('PLT ที่ใช้',fmt(r.plt_used,2)+' K/µL')}${dcell('Platelet yield',fmt(r.platelet_yield,3)+' ×10¹¹')}${dcell('Equivalent Units',fmt(r.equivalent_units,2))}${dcell('WBC ADAM',fmt(r.wbc_adam,4)+' /µL')}${dcell('Residual WBC',fmt(r.residual_wbc,3)+' ×10⁶')}${dcell('pH',fmt(r.ph_value,3))}${dcell('วันเวลาวัด pH',dateTH(r.ph_measured_at))}${dcell('ผู้บันทึก',profileName(r.created_by))}${dcell('ผู้ LOCK',profileName(r.locked_by))}</div>
